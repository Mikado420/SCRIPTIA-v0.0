      ...
      <div className="flex items-center space-x-2">
        ...
        {phase === 'ARCANA_PLACEMENT' && isMyTurn && !hasPlacedArcanaThisTurn && (
          <button
            ...
            className={`px-3 py-2.5 rounded-xl font-black text-xs transition-all shadow-lg active:scale-95 flex items-center space-x-1.5 border ${
              isHandSelected
          ...
          >
            ...
            <span>{isHandSelected ? 'アルカナチャージ' : 'アルカナ配置'}</span>
          </button>
        )}
        ...
        <button
          ...
          disabled={!canAct}
          onClick={onNextPhase}
          className={`relative group h-14 sm:h-16 px-4 sm:px-6 rounded-2xl font-black tracking-wider transition-all select-none shadow-2xl active:scale-95 flex items-center justify-center border-2 ${
            canAct
          ...
          style={{
            boxShadow: canAct
              ? '0 6px 0 rgba(0,0,0,0.6), 0 10px 20px rgba(0,0,0,0.4)'
              : '0 4px 0 rgba(0,0,0,0.8)',
            transform: canAct ? 'translateY(-2px)' : 'none',
          }}
        >
          {/* Internal 3D Glint */}
          <div className="absolute top-1 inset-x-2 h-1/3 rounded-t-xl bg-gradient-to-b from-white/30 to-transparent pointer-events-none" />

          {/* Label Content */}
          <div className="flex items-center space-x-1.5 z-10 text-xs sm:text-sm">
            {!isMyTurn ? (
              <>
